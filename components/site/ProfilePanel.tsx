...
export function ProfilePanelRail() {
  return (
    <aside
      ...
      className="no-print fixed inset-y-0 left-0 top-14 z-40 hidden w-[17rem] overflow-y-auto border-r border-hairline bg-paper px-7 py-10 lg:block"
    >
      <PanelBody />
    </aside>
  );
...
function PanelBody() {
  return (
    <div className="flex flex-col items-center text-center">
      <Link href="/" className="group relative grid size-40 place-items-center" aria-label="Home">
        ...
        <img
          ...
          width={460}
          height={460}
          className="size-[5.5rem] rounded-full border border-hairline object-cover shadow-sm transition-transform duration-500 group-hover:scale-105"
        />
      </Link>

      <Link href="/" className="serif mt-4 text-[1.4rem] leading-tight transition-colors hover:text-accent">
        {profile.name}
      </Link>

      <p className="mt-2 text-[0.8rem] leading-relaxed text-ink-2">{profile.subtitle}</p>
        ...
        {externalLinks.map(({ label, value, href, Icon }) => (
          <li key={label}>
            <a
              ...
              title={value}
              className="group flex items-center gap-2.5 rounded-md px-2 py-1.5 text-left text-[0.825rem] text-ink-2 transition-colors hover:bg-paper-2 hover:text-ink"
            >
              <Icon className="size-3.5 shrink-0 text-muted transition-colors group-hover:text-accent" />
